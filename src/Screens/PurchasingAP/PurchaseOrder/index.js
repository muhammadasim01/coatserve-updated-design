...
export default function PurchaseOrder() {
  ...
  React.useEffect(async () => {
    ...
    //     if (element.U_FormId === "OPOR") {
    //       console.log(element.U_Authorization);
  ...
  const ontextChanged = (e, item) => {
    ...
    itemDetail[item][e.target.name] = e.target.value;
    itemDetail[item]["UnitPrice"] =
      ((itemDetail[item].Circumference * itemDetail[item].FaceLength) / 100) *
      itemDetail[item].Rate;
    itemDetail[item]["AfterDiscount"] =
      itemDetail[item].No_Cylinder * itemDetail[item].UnitPrice -
      itemDetail[item].No_Cylinder *
        itemDetail[item].Discount *
        (itemDetail[item].Discount / 100);
    itemDetail[item]["Total_LC"] =
      itemDetail[item].AfterDiscount + itemDetail[item].TaxAmount;
  ...
  const DocLinesDropDownOnChange2 = (selectedItem, item, name) => {
    ...
    itemDetail[item]["TaxAmount"] =
      Number(itemDetail[item].PriceAfterDiscount) *
      (selectedItem.item.VatGroups_Lines[0].Rate / 100);
    // obj[index]['TaxAmount_LC'] = obj[index].LineTotal *(selectedItem.item.VatGroups_Lines[0].Rate / 100)
    itemDetail[item]["GrossPriceAfterDiscount"] =
      Number(itemDetail[item].PriceAfterDiscount) + itemDetail[item].TaxAmount;
    itemDetail[item]["Total_LC"] = Number(
    ...
    itemDetail[item]["TaxTotalAmount"] =
      ...
      (selectedItem.item.VatGroups_Lines[0].Rate / 100);
    itemDetail[item]["GrossTotal"] =
      itemDetail[item].Total_LC + itemDetail[item].TaxTotalAmount;
    // SelectedItems.forEach(element => {
    //   SelectedDocLines.push({
    //     Total_LC: ItemsDetails[element]['Total_LC'],
    ...
    itemDetail.forEach((element) => {
      if (element.ItemCode) {
        console.log("element", element);
        sum = sum + Number([element.Total_LC]);
        sum2 = sum2 + Number([element.TaxTotalAmount]);
      }
    });
    setgetnetTotal(sum);
    setgettaxtotal(sum2);
  ...
  const QuantityFun = (e, index) => {
    ...
    doclines[index]["Total_LC"] =
      doclines[index].PriceAfterVAT * doclines[index].RemainingOpenQuantity;
    setPQDocumentLines(doclines);
  };
                            ...
                            getautounitprice[item.ItemCode] ? (
                              ...
                              <div style={{ width: "14rem", height: "auto" }}>
                                <input
                                  ...
                                  aria-describedby="Requesterid"
                                  value={
                                    item.ItemCode ? item.PriceAfterDiscount : ""
                                  }
                                />
                              </div>
                            )}
                            ...
                            getautounitprice[item.ItemCode] ? (
                              <div style={{ width: "14rem", height: "auto" }}>
                                <input
                                  ...
                                  value={
                                    item.ItemCode
                                      ? (item.TaxAmount + getautounitprice &&
                                          getautounitprice[item.ItemCode]
                                            .Price) * item.Quantity
                                      : ""
                                  }
