...
import { Button, Card, CardGroup, Table, Container,Tabs,Tab, Row, Col, Modal,Form } from "react-bootstrap";
...
const [currentComDate, setcurrentComDate] = React.useState(new Date().getFullYear() + '-' +(new Date().getMonth() + 1) + '-' + new Date().getDate())
  ...
  const GetItems = async (cookie) => {
    ...
    // setLoading(true)
    // -------------------------    Items API GET DATA   ------------------------------------------------------
    await axios.post(API_TYPES.GET, { api: LINKS.sap.PurchaseQuotation.purchaseItems, cookie: cookie }).then(function (ItemsResponse) {
  ...
  const GRpo = async (cookie) => {
    ...
    await axios.post(
      API_TYPES.GET,
      { api: LINKS.sap.MasterData.ActivePurchaseItems, cookie: cookie }
    ).then(function (res) {
  ...
  const GetRequesterDetail = async (cookie, api) => {
    ...
    // -------------------------    Items API GET DATA   -----------------------------------------------------------
    await axios.post(API_TYPES.GET, { api: api, cookie: cookie })
      .then(function (res) {
        response = res
      }).catch(function (error) {
        console.log(error);
    ...
    if (res) {
      ...
      res.forEach(element => {
        if (element.UserCode) {
          ...
          dropdown.push({ id: element.EmployeeID, name: (element.FirstName + "  " + element.MiddleName + "  " + element.LastName) });
        }
...
getdocumenttype={getdocumenttype}
          ...
          <Container fluid>
            ...
            <Modal show={show} onHide={handleClose} className="Modal-big" size="lg">
              <Modal.Header closeButton>
                <Modal.Title id="example-modal-sizes-title-lg">Open Purchase Orders List</Modal.Title>
              </Modal.Header>
              <Modal.Body>
                {PrRe && (
                  <Table responsive>
              ...
              <Modal.Footer>
              <Button variant='secondary'  onClick={()=>{handleShowModalTwo();handleClose()}}>Select</Button>
        ...
        <Table responsive striped bordered hover>
          ...
          <tbody>
            ...
            <input
                ...
                <td>
                  {item.ItemDescription||item.U_Dscription}
                </td>
                ...
                <td>
                  {item.ProjectCode||item.U_Project}
                </td>
                <td>
                  <input
                    type='text'
                    readOnly
                    class='form-control'
                    defaultValue={item.U_LineComments||item.U_LineComments}
                    name='LineRemarks'
                    onChange={e => {
                      LineRemarksFun(e, index, PrReIndex)
                    }}
    ...
    Choose
          ...
          <Button variant='primary' onClick={handleClose2}>
            ...
            <div style={{ width: '23.5rem', height: 'auto' }}>
              <Select
                ...
                options={[{ label: "1 - Company", value: 1 }, { label: "2 - Customer", value: 2 }, { label: "3 - Vendor", value: 3 }]}
                onChange={e => {
          ...
          <div className="OIGP"
            ...
            >
              <a onClick={e => {
                ...
                }} style={{cursor:'pointer'}}> <svg class="svg-icon15" viewBox="0 0 20 20">
      ...
      <Tabs defaultActiveKey="Inward Gate Pass Row" transition={false} id="noanim-tab-example">
              ...
              <tr key={`${index}`} >
                ...
                <td>
                  <input
                    type='text'
                    class='form-control'
                    value={item.U_LineComments||item.U_LineComments}
                    name='LineRemarks'
                    onChange={e => {
                      LineRemarksFun(e,index,PrReIndex)
                    }}
          ...
          <tbody>
            {SearchDocumentLines.map((item, index) => (
              ...
              <tr key={`${index}`} >
                ...
                <td>
                {item.BaseEntry||item.U_BaseEntry}
                </td>
                <td>
                  {item.BaseLine||item.U_BaseLine}
                </td>
        ...
        <DIV3>
          <Container fluid>
            ...
            <Button type="submit" 
              ...
              >{ButtonName}</Button>
              <Button variant="secondary" style={{ marginLeft: '5%', marginTop: '10%' }}
               onClick={() => {
