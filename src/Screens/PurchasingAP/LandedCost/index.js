...
export default function Invoice() {
  ...
  const submitLCS = async () => {
    ...
    body["LandedCostsAllocationAccount"] =
      getvalues.LandedCostsAllocationAccount;
  ...
  const valuechangeamountfunction = async (e, index) => {
    ...
    obj[index]["CostFactor"] =
      (100 / Number(totalbforeDiscount)) * Number(obj[index]["Amount"]);
    ...
    obj.forEach((item) => {
      if (item.Amount && item.AllocationBy === "ab_Quantity") {
        ...
        item.Amount &&
        item.AllocationBy === "ab_CashValueBeforeCustoms"
      ) {
  ...
  const QTY_BC = (getqtyamount, getbcamount) => {
    ...
    obj.forEach((item) => {
      lines.push({
        ...
        Allocated_CostA:
          Number(getqtyamount / TotalQuantity) * item.Quantity +
          Number(getbcamount / Number(totalbforeDiscount)) * item.LineTotal,
        Expenditure:
          Number(getqtyamount / TotalQuantity) * item.Quantity +
          (Number(getbcamount / Number(totalbforeDiscount)) * item.LineTotal) /
            item.Quantity,
        WhsePrice:
          item.UnitPrice +
          (Number(getqtyamount / TotalQuantity) * item.Quantity +
            (Number(getbcamount / Number(totalbforeDiscount)) *
              item.LineTotal) /
              item.Quantity),
        LineTotal1:
          item.LineTotal +
    ...
    obj.forEach((item) => {
      lines.push({
        ...
        Allocated_CostA:
          ...
          Number(
            (Number(getacamount) /
              Number(totalbforeDiscount + Number(CustomValue))) *
              (item.LineTotal + Number(item.Customs_Value))
          ),
        Expenditure:
          Number(getqtyamount / TotalQuantity) * item.Quantity +
          Number(
        ...
        WhsePrice:
          item.UnitPrice +
          item.Customs_Value / item.Quantity +
          Number(getqtyamount / TotalQuantity) * item.Quantity +
          Number(
            (Number(getacamount) /
              Number(totalbforeDiscount + Number(CustomValue))) *
  ...
  const QTY_BC_AC = (getqtyamount, getbcamount, getacamount) => {
    ...
    console.log(
      "AC",
      Number(
        Number(getacamount) / Number(totalbforeDiscount + Number(CustomValue))
      )
    );
  ...
  const quantity = (getqtyamount) => {
    ...
    obj.forEach((item) => {
      ...
      lines.push({
        ...
        WarehouseCode: item.WarehouseCode,
        // Allocated_CostA:item.Allocated_CostA,
        Allocated_CostA: Number(
          Number(getqtyamount / TotalQuantity) * item.Quantity
        ),
        // Allocated_CostA:item.Allocated_CostA,
        Expenditure:
          Number(Number(getqtyamount / TotalQuantity) * item.Quantity) /
          item.Quantity,
        WhsePrice:
          item.UnitPrice +
          Number(Number(getqtyamount / TotalQuantity) * item.Quantity) /
  ...
  const valuechanger2221 = (e, index) => {
    ...
    let obj = LCDocumentLines;
    obj[index][e.target.name] = e.target.value;
    obj[index]["Customs_Value"] =
      Number(obj[index]["Projected_Customs"]) * Number(obj[index]["Quantity"]);
    obj[index]["LineTotal2"] =
      Number(obj[index]["LineTotal"]) + Number(obj[index]["Customs_Value"]);
    obj[index]["Total_Costs"] =
      Number(obj[index]["Quantity"]) * Number(obj[index]["Projected_Customs"]);
    obj[index]["Whse_Price"] =
      Number(obj[index]["BaseDocumentPrice"]) +
      Number(obj[index]["Projected_Customs"]);
    obj[index]["Whse_Price1"] =
      Number(obj[index]["BaseDocumentPrice"]) +
      Number(obj[index]["Expenditure"]) +
      Number(obj[index]["Projected_Customs"]);
    obj[index]["LineTotal1"] =
      Number(obj[index]["Customs_Value"]) +
      Number(obj[index]["Allocated_Cost"]) +
      Number(obj[index]["LineTotal"]);
    obj[index]["Total_Costs1"] =
      Number(obj[index]["Quantity"]) * Number(obj[index]["Projected_Customs"]) +
      Number(obj[index]["Quantity"]) * Number(obj[index]["Expenditure"]);
    let sum = 0;
    obj.forEach((element) => {
      sum = sum + Number(element.Customs_Value);
    });
  ...
  const valuechanger222 = (e, index) => {
    ...
    obj[index]["Total_Costs"] =
      Number(obj[index]["Quantity"]) * Number(obj[index]["Projected_Customs"]);
    obj[index]["Whse_Price"] =
      Number(obj[index]["UnitPrice"]) + Number(obj[index]["Projected_Customs"]);
    obj[index]["Whse_Price1"] =
      Number(obj[index]["UnitPrice"]) +
      Number(obj[index]["Expenditure"]) +
      Number(obj[index]["Projected_Customs"]);
    obj[index]["LineTotal1"] =
      Number(obj[index]["Customs_Value"]) +
  ...
  const valuechanger = (e, index) => {
    ...
    obj[index]["Whse_Price"] =
      Number(obj[index]["UnitPrice"]) + Number(obj[index]["Expenditure"]);
    obj[index]["Whse_Price1"] =
      Number(obj[index]["UnitPrice"]) +
      Number(obj[index]["Expenditure"]) +
      Number(obj[index]["Projected_Customs"]);
    obj[index]["Allocated_Cost"] =
      Number(obj[index]["Quantity"]) * Number(obj[index]["Expenditure"]);
    obj[index]["LineTotal2"] =
      Number(obj[index]["LineTotal"]) + Number(obj[index]["Allocated_Cost"]);
    obj[index]["LineTotal1"] =
      Number(obj[index]["Customs_Value"]) +
      Number(obj[index]["Allocated_Cost"]) +
      Number(obj[index]["LineTotal"]);
  ...
  const submitLC = async () => {
    ...
    PDNDocumentLines.forEach((element) => {
      ItemLines.push({
        ...
        TotalCosts:
          Number(element.Allocated_CostA) + Number(element.Customs_Value),
    ...
    LaCost.forEach((element) => {
      if (element.AllocationBy && element.Amount) {
        CostLines.push({
          ...
          AllocationBy:
            element.AllocationBy === "ab_CashValueBeforeCustoms"
                ...
                <Tab eventKey="Items" title="Items">
                  {PDNDocumentLines && (
                    <Table responsive striped bordered hover>
                      ...
                      <tbody>
                        {PDNDocumentLines &&
                          PDNDocumentLines.map((item, index) => (
                            <tr key={`${index}`}>
                              <TD>{index + 1}</TD>
                              <TD>{item.ItemCode}</TD>
                              <TD>{item.ItemDescription}</TD>
                  ...
                  {LCDocumentLines && (
                    <Table responsive striped bordered hover>
                      ...
                      <tbody>
                        {LCDocumentLines &&
                          LCDocumentLines.map((item, index) => (
                            <tr key={`${index}`}>
                              <TD>{index + 1}</TD>
                  ...
                  {SearchItemLines && (
                    <Table responsive striped bordered hover>
                      ...
                      <tbody>
                        {SearchItemLines &&
                          SearchItemLines.map((item, index) => (
                            <tr key={`${index}`}>
                              <TD>{index + 1}</TD>
                              <TD>{item.VendorCode}</TD>
                              <TD>{item.Quantity}</TD>
                  ...
                  <Table responsive>
                    ...
                    <tbody>
                      ...
                      <tr>
                        ...
                        <TD>
                          {" "}
                          <div style={{ height: "auto" }} variant="primary">
                            <Select
                              menuPortalTarget={document.body}
