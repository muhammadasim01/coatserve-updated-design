...
export default function IncomingPayments() {
  ...
  const [PaymentInvoicesTotalAmountDue, setPaymentInvoicesTotalAmountDue] =
    React.useState(0);
  ...
  const [AccountinvoiceTotalAmount, setAccountinvoiceTotalAmount] =
    React.useState(0);
  ...
  const [getdocumentname, setgetdocumentname] = React.useState("VPM1");
  const [getdocumenttype, setgetdocumenttype] = React.useState("46");
  const [ModuleName, setModuleName] = React.useState("OutgoingPayments");
  ...
  React.useEffect(async () => {
    ...
    setButtonName("Add");
    setSearchNumber("0");
    setgetdocumentname("VPM1");
    setModuleName("OutgoingPayments");
    setgetdocumenttype("46");
    compareDate();
  ...
  const GetJEVendorData = async (cook) => {
    let body = {
      SqlCode: "GetVeOJE",
      SqlName: "GetVeOJE",
      SqlText: `Select BalDueDeb,TransType,DueDate, BalDueCred, Debit, Credit, ShortName, TransId From JDT1 Where TransType in (-2, 24,46, 30) and ShortName Like 'V%'`,
  ...
  const submit = async () => {
    ...
    if (getdoctype === "rAccount") {
      if (AccountselectedItems) {
        AccountselectedItems.forEach((element) => {
          PaymentAccountsdocLines.push({
            ...
            // Currency: 'PKR',
            // CountryCode: ItemsDetails[element]['Country']=== undefined ? "PK" :ItemsDetails[element]['Country'],
            ...
            GrossAmount:
              Number(ItemsDetails[element]["Net_Amount"]) +
              (Number(AccountinvoiceTotalAmount) * Number(TotalWTaxAmount)) /
                100,
            VatAmount:
              (Number(AccountinvoiceTotalAmount) * Number(TotalWTaxAmount)) /
              100,
      ...
      if (selectedItems) {
        selectedItems.forEach((element) => {
          PaymentMeansdocLines.push({
            ...
            Currency: "PKR",
            CountryCode:
              ItemsDetails[element]["Country"] === undefined
                ? "PK"
                : ItemsDetails[element]["Country"],
            CheckAccount: ItemsDetails[element]["Code"],
            Branch: getbranchaccount,
            AccounttNum: getcheckAccountNo,
            ...
            // "BankCode": "UBL",
            // "Branch": "0492",
            // "AccounttNum": "000232895043",
            // "CheckSum": 1000.0,
            // "Currency": "PKR",
            // "CountryCode": "PK",
            // // "CheckAbsEntry": 8,
            // "CheckAccount": "A20702004",
          });
        });
      }
      body = {
        ...
        CashSum:
          getcashaccount === undefined ? undefined : Number(getcashtotal),
        ...
        PaymentChecks:
          PaymentMeansdocLines.length > 0 ? PaymentMeansdocLines : [],
        PaymentAccounts: PaymentAccountsdocLines,
        TransferAccount:
          getTransferAccount === undefined ? undefined : getTransferAccount,
        TransferSum:
          gettransfertotal === undefined ? undefined : Number(gettransfertotal),
        TransferDate:
          getTransferDate === undefined ? undefined : getTransferDate,
        TransferReference:
          getReference === undefined ? undefined : getReference,
      };
      ...
      PQDocumentLines.forEach((element) => {
        if (element.isSelected) {
          ...
          IncomingPaymentdocLines.push({
            ...
            AppliedSys:
              Number(element.TotalPayment) === undefined
                ? element.DocTotal - element.PaidToDate
                : Number(element.TotalPayment),
      ...
      (ModalName === undefined &&
        ...
        getdoctype === "rCustomer") ||
      "rSupplier"
    ) {
      console.log("Customer Or Vendor");
      if (selectedItems) {
        selectedItems.forEach((element) => {
          PaymentMeansdocLines.push({
            // "LineNum":ItemsDetails[element][''],
  ...
  const GetPaymentOnAccountData = async (cook) => {
    let body = {
      SqlCode: "GetPayOnAc",
      SqlName: "GetPayOnAc",
      SqlText: `select NoDocSum,DocEntry,DocNum from OVPM`,
    };
    let api = `SQLQueries`;
    if (cook) {
      await axios
  ...
  const PMtabscontroller = async (value) => {
    let cook = await localStorage.getItem("cookie");
    let sapAPi = `SQLQueries('GetPayOnAc')/List`;
    await axios
      .post(API_TYPES.GET, { api: sapAPi, cookie: cook })
      .then(function (res) {
        console.log("res", res);
      ...
      .catch({});
    if (
      Array.isArray(value.PaymentChecks) &&
      value.PaymentChecks.length > 0 &&
      value.CashAccount &&
      value.CashSum &&
  ...
  const SearchPRNumberFilter = async () => {
    let sapAPi = `VendorPayments?$filter=DocNum eq ${SearchNumber}`;
    let cook = await localStorage.getItem("cookie");
    await axios
      .post(API_TYPES.GET, { api: sapAPi, cookie: cook })
      .then(async function (res) {
        ...
        setSearchDocumentLines(res.data.value[0].PaymentInvoices);
        let sum = 0;
  ...
  const ObjectTypePaymentInvoices = async (docnum, doctype) => {
    ...
    if (doctype === "it_PurchaseInvoice") {
      let sapAPi = `PurchaseInvoices?$filter=DocNum eq ${docnum}`;
      let cook = await localStorage.getItem("cookie");
      await axios
        .post(API_TYPES.GET, { api: sapAPi, cookie: cook })
        .then(function (res) {
          ...
          setInvoicesLines(res.data.value[0].DocumentLines);
          let sum = 0;
        ...
        .catch({});
      setShow4(true);
    } else if (doctype === "it_PaymentAdvice") {
      let sapAPi = `JournalEntries?$filter=OriginalJournal eq 'ttVendorPayment' and JdtNum eq ${docnum}`;
      console.log(sapAPi);
      let cook = await localStorage.getItem("cookie");
      await axios
        .post(API_TYPES.GET, { api: sapAPi, cookie: cook })
    ...
    prData.forEach((element) => {
      if (element.isSelected) {
        sum =
          ...
          (element.DocTotal
            ? element.DocTotal - element.PaidToDate
            : element.BalDueCred - element.BalDueDeb);
  ...
  const getDoctotalvalues = (e, index) => {
    ...
    prData.forEach((element) => {
      if (element.isSelected) {
        console.log(element.PaidToDate);
        sum =
          sum +
          Number(
        ...
        // sum = sum + Number( element.TotalPayment === undefined ? (element.DocTotal - element.PaidToDate) : element.TotalPayment)
      }
    });
    console.log(sum);
    setTotalAmountDue(sum);
  ...
  const getBatchQuantities = async (data) => {
    let cook = await localStorage.getItem("cookie");
    let sapAPi = `SQLQueries('GetVeOJE')/List`;
    await axios
      .post(API_TYPES.GET, { api: sapAPi, cookie: cook })
      .then(function (res) {
        console.log("res", res);
        if (res.data.value) {
          let filterddata = res.data.value.filter((el) => {
            return (
              el.ShortName === SelectedVendor &&
              el.BalDueCred - el.BalDueDeb !== 0
            );
          });
  ...
  return (
    <>
      ...
      />
      <h1 style={{ textAlign: "center" }}>Outgoing Payments</h1>
      <div className="main_container">
        <div className="left">
          <div className="header_items_container">
            ...
            <Select
              placeholder={
                HeaderData && HeaderData.DocType === "rCustomer" ? (
                  <p>Customer</p>
          ...
          {getAccountvalue ? (
            <>
              ...
              <div className="header_items_container">
                ...
                <Select
                  isMulti
                  placeholder={
                    getPaymentchecksdata && getPaymentchecksdata.CheckAccount
                  }
            ...
            <>
              ...
              <div className="header_items_container" onClick={ContactPerson}>
            ...
            <Modal.Body>
              <Form>
                  ...
                  <DIV3>
                    <Container fluid>
                      ...
                      <div style={{ width: "23.5rem", height: "auto" }}>
                        <Select
                          placeholder={
                            InvoicesHeaderData && InvoicesHeaderData.CardCode
                          }
                      ...
                      <div style={{ width: "23.5rem", height: "auto" }}>
                        <Select
                          placeholder={
                            InvoicesHeaderData &&
                            InvoicesHeaderData.ContactPersonCode
                          }
                          displayValue="name" // Property name to display in the dropdown options
                        />
                      </div>
                      ...
                      <div style={{ width: "23.5rem", height: "auto" }}>
                        ...
                        <input
                          defaultValue={
                            InvoicesHeaderData && InvoicesHeaderData.NumAtCard
                          }
                          type="text"
                          class="form-control"
                          readOnly
                    ...
                    <Container style={{ marginLeft: "43rem" }}>
                      ...
                      >
                        ...
                        <input
                          ...
                          readOnly="readOnly"
                          defaultValue={
                            InvoicesHeaderData && InvoicesHeaderData.DocNum
                          }
                          class="form-control"
                        />
                      </div>
                      ...
                      >
                        ...
                        <input
                          ...
                          defaultValue={
                            InvoicesHeaderData &&
                            InvoicesHeaderData.DocumentStatus
                          }
                        />
                      </div>
                      <div
                      ...
                      >
                        ...
                        <input
                          ...
                          class="form-control"
                          defaultValue={
                            InvoicesHeaderData && InvoicesHeaderData.DocDate
                          }
                        />
                      </div>
                      <div
                      ...
                      >
                        ...
                        <input
                          ...
                          class="form-control"
                          defaultValue={
                            InvoicesHeaderData && InvoicesHeaderData.DocDueDate
                          }
                        />
                      </div>
                      <div
                      ...
                      >
                        ...
                        <input
                          ...
                          class="form-control"
                          defaultValue={
                            InvoicesHeaderData && InvoicesHeaderData.TaxDate
                          }
                        />
                      </div>
                      <br />
                ...
                >
                  ...
                  <Tab eventKey="Attachment" title="Attachment"></Tab>
                </Tabs>
                {/* Bottom Side+++++++++ */}
                <CardGroup>
                  <DIV3>
                    <Container fluid>
                      <div style={{ width: "23.5em", height: "auto" }}>
                        ...
                        <input
                          ...
                          defaultValue={
                            InvoicesHeaderData &&
                            InvoicesHeaderData.SalesPersonCode
                          }
                        />
                      </div>
                      <div style={{ width: "23.5rem", height: "auto" }}>
                        ...
                        <Select
                          placeholder={
                            InvoicesHeaderData &&
                            InvoicesHeaderData.DocumentsOwner
                          }
                          displayValue="name" // Property name to display in the dropdown options
                        />
                      </div>
                  ...
                  <DIV3>
                        ...
                        <CardGroup>
                          <div style={{ width: "11.75rem" }}>
                            <input
                              ...
                              value={
                                InvoicesHeaderData &&
                                InvoicesHeaderData.DiscountPercent
                              }
                              class="form-control"
                              name="TaxAmount(LC)"
                            />
                          ...
                          <div style={{ width: "11.75rem" }}>
                            <input
                              ...
                              value={
                                InvoicesHeaderData &&
                                InvoicesHeaderData.TotalDiscountSC
                              }
                            />
                          </div>
                        </CardGroup>
                      ...
                      <CardGroup>
                        <div style={{ width: "11.75rem" }}>
                          <input
                            ...
                            value={
                              InvoicesHeaderData &&
                              InvoicesHeaderData.DownPaymentPercentage
                            }
                          />
                        </div>
                        <div style={{ width: "11.75rem" }}>
                          <input
                            ...
                            value={
                              InvoicesHeaderData &&
                              InvoicesHeaderData.DownPaymentAmountSC
                            }
                          />
                        </div>
                      </CardGroup>
                    ...
                    <div style={{ width: "23.5rem", marginLeft: "4rem" }}>
                      ...
                      <input
                        ...
                        class="form-control"
                        value={
                          InvoicesHeaderData && InvoicesHeaderData.VatSumSys
                        }
                        name="TaxAmount(LC)"
                      />
                    </div>
                    <div style={{ width: "23.5rem", marginLeft: "4rem" }}>
                      ...
                      <input
                        type="text"
                        value={
                          InvoicesHeaderData && InvoicesHeaderData.DocTotal
                        }
                        placeholder=""
                        class="form-control"
                      />
                    ...
                    <div style={{ width: "23.5rem", marginLeft: "4rem" }}>
                      ...
                      <input
                        type="text"
                        value={
                          InvoicesHeaderData && InvoicesHeaderData.DocTotalSys
                        }
                        placeholder=""
                        class="form-control"
                      />
      ...
      >
        <Tab eventKey="Contents" title="Contents">
                  ...
                  <tr key={`${index}`}>
                    ...
                    <TD>
                      ...
                      <input
                        ...
                        class="form-control"
                        onChange={(e) => {
                          setAccountinvoiceTotalAmount(e.target.value);
                          gettableinputvalues(e, item);
                        }}
                      ></input>
                    </TD>
          ...
          {PrRe && (
            <Table responsive>
              {Array.isArray(PrRe) && PrRe.length > 0 ? (
                <TableHead>
                  <tr>
                    <th>Selected</th>
                    <th>Document No.</th>
              ...
              <tbody>
                {PrRe.map((item, index) => (
                  <tr key={`${index}`}>
                    ...
                    <TD>
                      {item.DownPaymentType === "dptInvoice" ? "IN" : "JE"}
                    </TD>
                    <TD>{item.DocDate || item.DueDate}</TD>
                    <TD>{item.Comments}</TD>
                    <TD>{item.DocTotal || item.Credit - item.Debit}</TD>
                    ...
                    <TD>
                      <input
                        ...
                        defaultValue={
                          ...
                          item.BalDueCred - item.BalDueDeb
                        }
                        onChange={(e) => {
                          getDoctotalvalues(e, index);
                        }}
                        class="form-control"
                      />
                    </TD>
              ...
              <tbody>
                {SearchDocumentLines.map((item, index) => (
                  <tr key={`${index}`}>
                    ...
                    <TD>
                      {item.InvoiceType === "it_PurchaseInvoice"
                        ? "PU"
                        : item.InvoiceType === "it_PaymentAdvice"
                        ? "JE"
                        : null}
                    </TD>
                    <TD>{item.DueDate}</TD>
              ...
              <Container fluid>
                <div style={{ width: "23.5rem", height: "auto" }}>
                  ...
                  <input
                    ...
                    defaultValue={
                      "PKR" ||
                      (getPaymentchecksdata && getPaymentchecksdata.Currency)
                    }
                    class="form-control"
                  ></input>
                </div>
                      ...
                      <Select
                        ...
                        placeholder={
                          (getPaymentchecksdata &&
                            getPaymentchecksdata.CheckAccount) ||
                          (HeaderData && HeaderData.CheckAccount)
                        }
                        options={ItemsDropDown}
                        onChange={ItemsDropDownfunc}
                        displayValue="name"
                        ...
                        <Container fluid>
                          <div style={{ width: "22rem", height: "auto" }}>
                            ...
                            <Select
                              placeholder={
                                (getPaymentchecksdata &&
                                  getPaymentchecksdata.CheckAccount) ||
                                (HeaderData && HeaderData.TransferAccount)
                              }
                              options={ItemsDropDown}
                              onChange={(e) => {
                                setgetTransferAccount(e.value);
                              }}
                              displayValue="name"
                            />
                          </div>
                          <div style={{ width: "22rem", height: "auto" }}>
                            ...
                            <input
                              ...
                              class="form-control"
                              onChange={(e) => {
                                setgetTransferDate(e.target.value);
                              }}
                              defaultValue={
                                (getPaymentchecksdata &&
                                  getPaymentchecksdata.DueDate) ||
                                (HeaderData && HeaderData.TransferDate)
                              }
                            />
                          </div>
                          <div style={{ width: "22rem", height: "auto" }}>
                            ...
                            <input
                              name={"Reference"}
                              onChange={(e) => {
                                setgetReference(e.target.value);
                              }}
                              defaultValue={
                                HeaderData && HeaderData.TransferReference
                              }
                              type="text"
                              class="form-control"
                            />
                        ...
                        <Container fluid>
                          ...
                          <div style={{ width: "22rem", height: "auto" }}>
                            ...
                            <input
                              ...
                              class="form-control"
                              defaultValue={
                                HeaderData && HeaderData.TransferSum
                              }
                              onChange={(e) => {
                          ...
                          <div style={{ width: "22rem", height: "auto" }}>
                            ...
                            <Select
                              placeholder={
                                (getPaymentchecksdata &&
                                  getPaymentchecksdata.CheckAccount) ||
                                (HeaderData && HeaderData.CashAccount)
                              }
                              options={ItemsDropDown}
                              onChange={(e) => {
                                setgetcashaccount(e.value);
                              }}
                              displayValue="name"
                            />
                          </div>
                  ...
                  <Container fluid>
                    ...
                    <div style={{ width: "23.5rem", height: "auto" }}>
                      ...
                      <input
                        ...
                        type="number"
                        value={
                          Number(AccountinvoiceTotalAmount) +
                            Number(
                              (Number(AccountinvoiceTotalAmount) *
                                Number(TotalWTaxAmount)) /
                                100
                          ...
                          Number(TotalAmountDue) -
                            Number(TotalWTaxAmount) +
                            Number(getpaymentonAccount) ||
                          PaymentInvoicesTotalAmountDue + getpaymentonAccount
                        }

                        // defaultValue={
                        //  ( Number(TotalAmountDue) - Number(TotalWTaxAmount) + Number(getpaymentonAccount)) || PaymentInvoicesTotalAmountDue
                        // }
                      />
                    </div>
                    <div style={{ width: "23.5rem", height: "auto" }}>
                        ...
                        value={
                          HeaderData && HeaderData.DocNum
                            ...
                            : Number(TotalAmountDue) -
                              Number(TotalWTaxAmount) +
                              Number(getpaymentonAccount) -
                              (Number(gettransfertotal) +
                                Number(getcheckamount) +
                                Number(getcashtotal)) +
                              Number(getbankcharge)
                        }
                      />
                    </div>
                    <div style={{ width: "23.5rem", height: "auto" }}>
          ...
          <>
            <div className="right">
              ...
              <div className="header_items_container">
                ...
                <input type="number" name="Discount%" class="input" readOnly />
              </div>
              <div className="header_items_container">
                <label>
                  Total Amount Due{" "}
