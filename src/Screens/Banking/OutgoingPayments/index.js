...
const [currentComDate, setcurrentComDate] = React.useState(new Date().getFullYear() + '-' +(new Date().getMonth() + 1) + '-' + new Date().getDate())
      ...
      AccountselectedItems.forEach(element => {
        PaymentAccountsdocLines.push({
          ...
          //  AccounttNum: getcheckAccountNo,
             ...
             "VatAmount": (Number(AccountinvoiceTotalAmount) * Number(TotalWTaxAmount)/100)
        })
  ...
  const checkboxSelect = (item, index) => {
    ...
    prData.forEach(element => {
      if (element.isSelected) {
        sum = sum + (element.DocTotal ? (element.DocTotal - element.PaidToDate) : (element.BalDueCred - element.BalDueDeb))
...
console.log("PrRe",prData)
    ...
    prData.forEach(element => {
      if (element.isSelected) {
        ...
        sum = sum + Number( element.TotalPayment === undefined ? ((element.DocTotal - element.PaidToDate) || (element.BalDueCred - element.BalDueDeb)) : element.TotalPayment)
  ...
  const getBatchQuantities = async (data) => {
    ...
    await axios
      ...
      .then(function (res) {
        console.log("res",res)
        if(res.data.value){
          let filterddata = res.data.value.filter(el => {return el.ShortName  === SelectedVendor && el.BalDueCred - el.BalDueDeb !== 0})
              ...
              <tbody>
                {PrRe.map((item, index) => (
                  <tr key={`${index}`}>
                    ...
                    type='text'
                    name='TotalPayment'
                    defaultValue={item.DocTotal - item.PaidToDate || item.BalDueCred - item.BalDueDeb}
              ...
              <tbody>
                ...
                >
                  ...
                  <GrDocumentPerformance />
                    ...
                    {item.DocEntry}</TD>
                    <TD>
                    {item.InvoiceType === 'it_PurchaseInvoice' ?   "PU"
                              : item.InvoiceType === 'it_PaymentAdvice' ?  "JE"
                              : null}
                   </TD>
                    <TD>{item.DueDate}</TD>
                    <TD>{item.Comments}</TD>
