...
const [currentComDate, setcurrentComDate] = React.useState(new Date().getFullYear() + '-' +(new Date().getMonth() + 1) + '-' + new Date().getDate())
  ...
  const GetAccountBalanceAll = async cook => {
    let body = {
      "SqlCode": "GetAllJDT1",
      "SqlName": "GetAllJDT1", 
      "SqlText": `Select ContraAct,TransType,Project,DueDate,BalFcDeb,BaseRef,BalFcCred, BalDueCred,BalDueDeb, Debit, Credit,FCDebit,FCCredit,Account,Ref3Line,Ref2,Ref1,TaxDate, ShortName,LineMemo ,TransId From JDT1 `
  ...
  const getSQLQueryResponceAll = async () => {
    let cook = await localStorage.getItem('cookie');
    let sapAPi = `SQLQueries('GetAllJDT1')/List`
    await axios
  ...
  const handleShow = async () => {
    if(getdoctype=== 'rCustomer'){
      if(getalldata){
          let filterddata = getalldata.filter(el => {return el.ShortName  === SelectedVendor && el.BalDueDeb - el.BalDueCred !== 0})
          let Doclines = []
          let sum = 0
      ...
      if(getalldata){
        let filterddata = getalldata.filter(el => {return el.ContraAct  === AccountselectedItems && el.BalDueDeb - el.BalDueCred !== 0})
          let Doclines = []
          let sum = 0
          filterddata.forEach(element=>{
            Doclines.push({
    ...
    if(getalldata){
      let filterddata = getalldata.filter(el => {return el.Project  === getProjectCode && el.BalDueDeb - el.BalDueCred !== 0})
        let Doclines = []
        let sum = 0
        filterddata.forEach(element=>{
          Doclines.push({
  ...
  return (
    <>
      ...
      />
      <h1 style={{ textAlign: 'center' }}>Account Balance Report</h1>
      ...
      { 
                      <Table responsive striped bordered hover id="table-to-xls">
                        <TableHead>
                          <tr>
                            ...
                            <th>Ref. 3</th>
                            <th>Details</th>
                            <th>Balance Due (FC)</th>
                            <th>Balance Due (LC)</th>
                            <th>C/D (FC)</th>
                            <th>C/D (LC)</th>
                            <th>Credit (LC)</th>
                            <th>Cumulative Balance (LC)</th>
                            <th>Debit (LC)</th>
                        ...
                        <tbody>
                          {getfilterddata && getfilterddata.map((item, index) => (
                            <tr key={`${index}`}>
                              <TD>{index + 1}</TD>
                              <TD> <div style={{ width: '11rem', height: 'auto' }}>
                                {item.TransType === "-2" ? ("OB") : item.TransType === "24" ? ("RC") : item.TransType === "30" ? ("JE") : item.TransType === "13" ? ("IN"): item.TransType === "14" ? ("CN"): item.TransType === "46" ? ("PS"):item.TransType === "18" ? ("PU"):null}
                                </div></TD>
                                <TD>
                  ...
                  onClick={() => ObjectTypePaymentInvoices(item.TransId,item.BaseRef)}
                >
                  {' '}
                  <GrDocumentPerformance />
                    ...
                    {item.TransId }</div></TD>
                              ...
                              <TD> <div style={{width: '11rem', height: 'auto' }}>
                              {Number(item.BalFcDeb) + Number(item.BalFcCred)}
                                </div>
                              </TD>
                              <TD> { item.BalDueDeb !== 0 ? (<div style={{width: '11rem', height: 'auto' }}>
                               PKR {item.BalDueDeb + item.BalDueCred}
                                </div>) :(<div style={{ color:'green',width: '11rem', height: 'auto' }}>
                               PKR ( {item.BalDueDeb + item.BalDueCred} )
                                </div>)
                                }
                              </TD>
                                <TD> <div style={{ width: '11rem', height: 'auto' }}>
                                {item.FCDebit + item.FCCredit}
                                </div></TD>
                              <TD> { item.Debit !== 0 ? (<div style={{width: '11rem', height: 'auto' }}>
                               PKR {item.Debit + item.Credit}
                                </div>) :(<div style={{ color:'green',width: '11rem', height: 'auto' }}>
                               PKR ( {item.Debit + item.Credit} )
                                </div>)
                                }
                        ...
                        <CardGroup>
                          ...
                          >
                            ...
                            <input
                              ...
                              name='Remarks'
                              class='form-control'
                              defaultValue={currentDate || HeaderData && HeaderData.Memo}
                            ></input>
                          </div>
                        </CardGroup>
                        <CardGroup>
      ...
      <CardGroup>
        ...
        <DIV3>
        <Container fluid>
        <div style={{ width: '23.5rem', height: 'auto',marginTop:'5rem' }} >
          <label>Opening Balance </label>
              <input
                  type='number'
                  name='BlanceDue' 
                  value={getfilterddata&&getfilterddata[0].ComulativeBalance}
                  readOnly
                  class='form-control'
                /> 
              </div>
          ...
          <label>Closing Balance</label>
              <input
                  ...
                  name='BlanceDue' 
                  readOnly
                  value={getfilterddata&&getfilterddata[getfilterddata.length-1].ComulativeBalance}
                  class='form-control'
                /> 
              </div>
          </Container>
