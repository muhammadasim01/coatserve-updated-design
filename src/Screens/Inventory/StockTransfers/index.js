    ...
    selectedItems.forEach((element) => {
      element.ItemWarehouseInfoCollection.forEach((element2) => {
        BatchNumberQuantityData.forEach((docElement) => {
          if (
            element2.ItemCode === docElement.ItemCode &&
            element2.WarehouseCode === docElement.WhsCode
          ) {
            Batchlines.push({
    ...
    PQDocumentLines.forEach((element) => {
      BatchNumberQuantityData.forEach((docElement) => {
        if (
          ...
          element.WarehouseCode === docElement.WhsCode
        ) {
          Batchlines.push({
            BaseLineNumber: index,
            AbsEntry: docElement.AbsEntry,
      ...
      getdata.forEach((element) => {
        docLines.push({
          ...
          BatchNumbers: docLines2.filter(function (i) {
            return i.ItemCode == element.ItemCode;
          }),
        });
      });
      let body = {
                    ...
                    <div style={{ width: "11.5rem", height: "auto" }}>
                      <Select
                        ...
                        onChange={(e) => {
                          ...
                          getwareHousesOfBranch(e.item.Name);
                        }} // Function will trigger on select event
