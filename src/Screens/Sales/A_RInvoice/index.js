...
const [currentComDate, setcurrentComDate] = React.useState(new Date().getFullYear() + '-' +(new Date().getMonth() + 1) + '-' + new Date().getDate())
  ...
  const BatchQuantitiesfunction = async (item,index) => {
    ...
    await axios
      ...
      .then(function (res) {
        ...
        setTimeout(function () { modalshow(item,index,res.data.value) }, 2000);
        }else{
        }
      ...
      if (element.isSelected) {
        element.DocumentLines.forEach(item2 => {
          BatchNumberQuantityData.forEach(docElement => {
            ...
            if (item2.ItemCode === docElement.ItemCode && item2.WarehouseCode === docElement.WhsCode) {
              Batchlines.push({
  ...
  const handleShow = async () => {
    ...
    if (getcopyfromvalue === "SO") {
      let SAPapi = `Orders?$filter=DocumentStatus eq 'bost_Open' and CardCode eq '${SelectedVendor}'`;
      await axios
        .post(API_TYPES.GET, {
          api: SAPapi,
          cookie: cookie
      ...
      let SAPapi = `DeliveryNotes?$filter=DocumentStatus eq 'bost_Open' and CardCode eq '${SelectedVendor}'`;
      await axios
        .post(API_TYPES.GET, {
          api: SAPapi,
          cookie: cookie
        ...
        element.DocumentLines.forEach(element2 => {
          if (element2.isSelected && element2.Variance === "Excess") {
            DocLines.push({
              ...
              BatchNumbers: docLines2.filter(function (i) { return i.ItemCode == element2.ItemCode;})
            })
        ...
        <CardGroup>
         ...
         <br/>
              ...
              <Modal show={show} onHide={handleClose} className="Modal-big" size="lg">
                <Modal.Header closeButton>
                  <Modal.Title id="example-modal-sizes-title-lg"> Document Numbers List</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                  {PrRe && (
                    <Table responsive>
              ...
              <Modal show={modalTwo === "modal-two"} className="Modal-big" size="lg">
                <Modal.Title id="example-modal-sizes-title-lg">List of Open Items</Modal.Title>
                <Modal.Body>
                  <Table responsive striped bordered hover>
                    <TableHead>
                      <tr>
          ...
          <DIV3>
            <Container>
              ...
              >
                ...
                }} style={{ cursor: 'pointer' }}> <svg class="svg-icon9" viewBox="0 0 20 20">
            ...
            <input
              ...
              <div style={{ width: '23.5rem', marginLeft: '3rem' }}>
                ...
                <input
                  ...
                  defaultValue={HeaderData && HeaderData.DocDueDate ? HeaderData && HeaderData.DocDueDate : getdocduedate}
                />
              </div>
              <div style={{ width: '23.5rem', marginLeft: '3rem' }}>
                    ...
                    PrReItem.DocumentLines.map((item, index) => (
                      ...
                      <tr key={`${index}`}>
                        ...
                        {Array.isArray(getbatchnumbers) && getbatchnumbers.length > 0 ? (<TD>
                          <Button onClick={(e) => { getBatchQuantities(item, index) }}> Batch </Button>
                        </TD>) : null}
                        <TD>
                          <div style={{ width: '14rem', height: 'auto' }}>
                            <input
                      ...
                      <TD>
                        <div style={{ width: '14rem' }}>
                          <Select
                            ...
                            options={TaxCode} // Options to display in the dropdown
                            onChange={e => {
                              SearchDocLinesDropDownOnChange2(e, index, 'VatGroup')
                            }}
                            // onSelect={CostCentre} // Function will trigger on select event
                            // onRemove={CostCentre} //Function will trigger on remove event
                            displayValue='name' // Property name to display in the dropdown options
        ...
        <CardGroup>
          ...
          <DIV3>
                ...
                <CardGroup>
                  <div style={{ width: '11.75rem' }}>
                    <input
                      ...
                      class='form-control'
                      name='TaxAmount(LC)'
                      defaultValue={disc || HeaderData && HeaderData.DiscountPercent}
                    />
                  </div>
                  <div style={{ width: '11.75rem' }}>
                    <input
            ...
            <div style={{ width: '23.5rem', marginLeft: '4rem' }}>
              ...
              <input type='text'
               value={((totalbforeDiscount - DiscountTotal) + gettaxtotal)  || HeaderData && (HeaderData.DocTotalFc != 0 ? HeaderData.DocTotalFc : HeaderData.DocTotal)}
